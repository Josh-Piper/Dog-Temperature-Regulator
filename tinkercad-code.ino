...
int blueLedPinOut = 2;
int greenLedPinOut = 3;
int redLedPinOut = 4;

int purpleFanPinOut = 13;
int brownHeaterPinOut = 12;

int temperaturePinInput = 0;
...
int DANGEROUS_DOG_TOO_COLD_TEMPERATURE = 18;
int DANGEROUS_DOG_TOO_HOT_TEMPERATURE = 30;
...
float getCelciusTemperature() {
    int temperatureinAnalog = analogRead(temperaturePinInput);
    float voltage = temperatureinAnalog * 4.68;
    voltage /= 1024.0;
    float celciusTemperature = (voltage - 0.5) * 100;
...
bool handleDogTooHot() {
    ...
    if (countTooHot != -1) {
		...
		if (hasSentAlertAlready == false) {
			Serial.println("initialising too hot");
        }
...
bool handleDogTooCold() {
    ...
    if (countTooCold != -1) {
        ...
        if (hasSentAlertAlready == false) {
			Serial.println("initialising too cold");
        }
...
void loop()
{
    ...
    if (isDogOptimalTemperature) {
        ...
        if (hasSentAlertAlready == true) {
          	Serial.println("Dog is back at a normal temperature. No need to worry! :-)");
